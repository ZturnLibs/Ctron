...
- 合法目标:`var` 绑定、经可变访问路径的 `var` 字段、切片/数组元素。可变性判定:目标路径的**根绑定**必须为 `var`,且沿途字段声明为 `var`(类引用穿越详见 §7.5)。
...
- `&&` 短路逻辑与;`!` 逻辑非;逻辑或写 `!(a && b)` 或两条件结构化(v0.3 无 `||`——`or` 已被占用为取默认,保留 `||` 不引入,防止二义)。比较不可链:`a < b && b < c`。
...
- 闭包类型参与 Send 判定:闭包是 Send 当且仅当全部捕获 Send(§7.4)。
- 跨任务传递闭包 = spawn 捕获检查(§7.4)。
