...
- **v0 限制(实现口径,非语言承诺)**:
  1. 整数字面量按期望类型宽度门(E2040,§3.7/§10):纯十进制无后缀字面量在 let 注解/赋值/返回/实参四点核对界内,超界拒绝;**I64 期望下超 I32 宽度的字面量直入 I64 域**(十进制取值)。`0x/0o/0b` 宽字面量与 comptime 域 v0 不进此门(运行期字面量求值超 I32 宽仍 panic)。
