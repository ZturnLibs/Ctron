...
- **v0 限制(实现口径,非语言承诺)**:
  1. 整数字面量按期望类型宽度门(E2040,§3.7/§10):无后缀字面量(十进制与 `0x/0o/0b` 同口径,下划线分隔符剥离)在 let 注解/赋值/返回/实参四点核对界内,超界拒绝;**I64 期望下超 I32 宽度的字面量直入 I64 域**(十进制取值)。comptime 域 v0 不进此门(运行期字面量求值超 I32 宽仍 panic)。
  2. `I64` 算术 v0 **无溢出检查**(文本域自然不溢出;发射侧 `int64_t` 运算未检查),即 §4.5"默认检查算术"对 `I64` 暂不生效。值超出 int64 宽度时解释侧(继续生长)与发射侧(int64)行为分歧,v0 测试夹具限定在 int64 宽度内。
