...
- **自举自用 struct 的工具链成熟度**:S0 尖峰先行验证(seed 解释编译器自源
  ...
  微修 + 维持现状」,已预留。
- **S0 结果(0926,已验收)**:编译器源内 struct(ValE+构造/字段读)自举两侧
  构建全通(seed 解释发射 + native),符号落二进制确认;工具链成熟度 ✓。
  注记:① Val 零分配的实证归 S1 的 AMEM 门(须先有 Val 才可量,尖峰期不可测);
  ② 解释口径下「用户 struct」= U 域编码列表(非零分配),与 eval 内部 Val
  表示是两回事,勿混淆。
