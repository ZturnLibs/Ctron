...
1. **json_write 并入 json**:历史切片分裂,违一域一模块;合并为破坏性演进,待裁决后 since 升版
   → **已裁决落地(2026-09-23,d144182)**:C17 规避解除实证(合并面 43 decls 宿主 test 10/10),写出半边归位 json.ct;宪章 v2 悬案条同步清账
2. **域目录形态收录**:http/net/db/tls/gui 皆已目录化("一域一目录、目录内零 use"),既成事实转正
   → **已裁决(2026-09-23 宪章 v2,6e7ad0c)**:一域一目录转正(宪章 #1);但"目录内零 use"改判为"域内 use 允许、禁环"(http/client、db/pg 既成事实口径;详见 2026-09-23-std-tiering-design.md §3)

### 8.4 裁决状态
- §3 范围裁定、§5 创新方向:**已按"继续"指令执行中**(parity 两臂落地、crypto 全链、对数式验收在 crypto/csv/工具对数持续)
- §8.3.2 域目录形态:**已随宪章 v2 裁决**(6e7ad0c;"目录内零 use"改判为域内 use 允许禁环);§8.3.1 json_write 并入:**已裁决落地**(d144182;C17 规避解除,宿主 10/10 复验绿)
