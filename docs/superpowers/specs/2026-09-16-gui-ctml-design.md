...
- `view X` 编译为:X 的**静态骨架**(元素树形状)+ **绑定槽表**(属性/文本/类到
...
1. **组合唯一**:组件之间互相引用、嵌套、分发内容,全部经组合完成——
   **组件间不设继承**(结构、行为、样式三个维度均无 view 级继承);
...
2. **Props 单向向下**:父经属性绑定传数据与回调,子只读;子组件**禁止**回写
...
失效是**事件边界**的批量动作,不做每表达式即时传播;细粒度体现在"只重算脏槽",
不体现在即时性。
...
- 监听 `.ct/.ctml` 变更 → 仅重编译变更模块的骨架/样式/绑定槽 → **状态树原址保留**,
  ...
  本条承诺的宿主是**解释口径**(§11.5),原生口径降级路径见同节。
...
- **M0–M2 能力边界(规范性)**:`GetCharPressed` 经平台字符路径可收 IME **上屏文本**
  ——"能输入中文、组词预编辑不可见"是本阶段的**定义形态**,非缺陷;M3 起补 preedit。
...
| 私有 DSL 表达式 | 跨语言摩擦(Slint 教训);Ctron 同源类型检查是结构性优势 |
...
> 2026-09-17 引擎对齐:T2 依赖的**浮层原语已进子集**(§5.1)——`select` 弹层、
...
1. **布局原因链(provenance,登记 M2 inspector 核心卖点)**:开发口径下布局桥为每个
   ...
   "它为什么在这"。业界空白;web devtools 的 box model 面板是唯一近似;
2. **确定性可快照**:布局 = 纯函数(骨架 + 样式 + 约束)→ 命令缓冲黄金差分(§11.7)。
   原生控件方案结构性做不到;这是自绘 + 自有布局桥的红利;
...
> comptime 预布局**收窄为布局静态验证**(E8191 静态溢出 / E8192 死 fill,W3);
