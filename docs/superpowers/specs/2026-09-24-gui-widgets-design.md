...
### 2.7 定时器/帧回调原语(P1,四轮补入;波次五b落地——注入时钟激活口径,真窗常亮消费者随 P2)
...
### 2.10 尺寸约束属性透出(P1,五轮补入;波次五b落地——max=GROW 钳制/min=FIT 托底/aspect P2)

- style 属性面扩容:`min-w`/`min-h`/`max-w`/`max-h`/`aspect`(Clay 已有,CTML
  未透)/`grow`(系数,现仅开关语义则升级)。纯属性透传,响应式布局地基。
- 夹具:s29 系相邻几何断言复用(x100/w100 口径已备)。

### 2.11 key 通道修饰键与事件路由(P1 地基,六轮补入;波次五b落地——修饰经 extern 读面零签名变更,表 match 在路由链第三位)
...
- **遮罩色 8 位制式**(波次三已落地销账):`#RRGGBBAA` 全面放行(6 位合法
  alpha=FF);g_parse_alpha + gui_alpha 置位/消费即复位通道,零 ABI 变更;
  C1 承诺修订为「#RRGGBB(A)」。
