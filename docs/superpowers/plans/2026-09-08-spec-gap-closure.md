...
| Phase 1 类型检查器 v0 | E2020 全量（变量读）、E2010 基础类型统一（注解/字面量/实参）、解析器 span 标注替换 JSON probe 定位、W8030/W8040、`?` 合法性检查 | 实施前另立计划 |
| Phase 2 comptime CVM | comptime fn 编译期执行、const 编译期求值、E6010 预算、E6030 parametricity、泛型单态化 v0 + 实例预算 | 同上 |
| Phase 3 模块与包 | use 生效、多文件程序、pub(pkg)/字段可见性、E5010 孤儿、E5020 循环依赖、Ctron.toml | 同上 |
