...
- **M1-b2 已交付(2026-09-19,s13_item 全绿,阶梯收编)**:each 体 = hbox 行模板
  (button 行级 + label),行级 on:click 按【实例下标】分派(3 行逐行点击下标精确)。
  实现:parse_each 模板子树整体记 each_nm(行组),draw 行组展开(容器每实例开合 +
  子模板随实例 emit,j 越界消费);教训入册——永真循环(ev_row.push 不动循环条件)、
  行模板双闭合消费、透明 bg 不产 RECT 命令(样式缺 bg 即静默不可点)。
