# Ctron C 版:C5 自举路线(优先目标;C5a✅ C5b✅ C5c✅ C5d①✅)
...
2. **C5d(①已完成)**:Ctron 词法器 v3(`lex_adv.ct`)加进制前缀/下划线/**小写数值后缀**(词边界)/`..`·`..=` 三字符/大写前缀回落(`0X10`→Int+Ident);单行综合输入**种类序列逐字一致**(`0xFFu8 .. 1_000 ..= 0o17 +% 7u8 * 0b1010u8 == 0X10`)。C5d②:字符串/插值/浮点与**换行过滤**(§1.6),差分输入扩到 `tests/*.ct`。
