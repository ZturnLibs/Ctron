# Ctron C 版:C5 自举路线(优先目标;C5a✅ C5b✅ C5c✅ C5d①✅ C5d②✅ C5e①✅ C5e②✅ C5f✅)
...
3. **C5e(完成)**:Ctron 词法器加浮点消歧/后缀收编;并新增 **payload 模式**差分(Ident/Int/Float
   记号文本逐字,后缀正确排除:2.5f64 → Float:2.5),suite_diff 6 用例全过。C5f:Ctron 词法器输出 **Str 原始源切片** payload
   (引号含入,双方切同一段源码 → 转义/插值天然逐字一致),修复驱动在比对前 free 源码的
   悬垂;suite_diff 7 用例全过。C5g:指数/进制小数 payload、多文件 corpus 扩面。
