...
### Todo 域包迁移尝试（2026-09-22，未落库——发现新问题）
...
- **终态验收（2026-09-22 晚）**：args11 hval=10 bg=11 全对；s19/s23 域包夹具绿；
  s23 发射口径骨架直通全绿。**剩余 4 旧形态夹具红（s3/s4/s5/s18）= peer 的
  parse_pkg 菱形依赖修复 + use 跨行落地（f15554c）与旧形态的交互**（s3 布局
  只产 2 命令；s18 scroll 裁剪失准）——非本泳道代码问题，登记归 parse/merge 泳道。
...
### P1 收口：Todo 域包迁移落库 + 视口剔除根因反转（bdb4265/94fec7b，2026-09-22 深夜）

- **迁移落地**：examples/todo 1242 → 130 行域包形态（bdb4265）——use std.gui 一行,
  用户树 = Box[Model] 活绑定 + act 动作收口 + on_key 键入钩子;双口径 headless
  test() 断言(键入/回显/添加/列表/空态/删除)+ --run run_kb 真窗口。差集如实
  登记:per-row checkbox/逐项删除 = 实例索引分发 L1 缺口(del 末项替代)。
- **根因反转(重要)**:上回迁移二试"when 分支不渲染"疑云**非 11 参桥**——
  系 **Clay 视口剔除**:headless 视口 320x240,内容增高后行/空态文本超出底边,
  Clay 不再为其生成渲染命令(trace 实证 gui_text 已调、命令缓冲缺失该条)。
  修复 = 视口 320x640。判定手法:CTRON_GUI_TRACE 分流 stderr + 命令缓冲转储
  对读,两源差集即被剔除元素。
- **新增阶梯夹具 s24_model_d**(94fec7b,阶梯 27→28):阶梯首个 Box 活模型 +
  when 态翻转 + each 增缩 + input 键入回显 + 多帧 bind 重入全组合夹具
  (s21 为静态绑定不覆盖活模型面);内嵌形态零 CWD 依赖。
- **顺手登记两坑**:①零参方法发射缺口——`.pop()` 发射为 t_pop(recv, 悬空逗号),
  泛化方法路径对零参方法未配 helper(归发射泳道;std/config.ct 的 .pop() 只走
  解释口径故未暴露),用户侧以重建列表+成员赋值配方绕行;②域包形态示例的
  run.sh 须设 CTRON_STDPATH(旧自包含形态不需要,缺失时 std 合并声明缺漏,
  cc 报隐式声明)。
- **验证**:todo run.sh headless 全链全绿(连两遍);s24 单跑全绿;阶梯 24/28
  (4 红为已登记 parse/merge 泳道项);e8 15/15;suite 73/73(宿主列既有分歧)。
