// ctron_gui.c —— S4:布局桥(承 S3)+ 事件队列 + 测试注入缝(计划 S4)
// 事件翻译(§12.3b 最小集):注入队列(测试)优先,raylib 轮询合并(headless 下惰性);
//   事件码:1=KeyDown 2=Click;边沿/命中测试归 Ctron 运行时(§11.3)
// shim 边界仍全 I32/Str(发射面裸 F32 参数/返回缺口修复前不变,见计划风险表)
...
// 取下一事件:0=无 1=KeyDown 2=Click(注入队列优先,再合并 raylib 轮询)
...
// ---- 命令读回(几何 ×100 整数;bg 为 RECTANGLE 命令背景色) ----
...
// ---- 绘制 flush(§12.3d 唯一绘制口;窗口口径交互验收用) ----
