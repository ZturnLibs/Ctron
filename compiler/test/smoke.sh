...
grep -q 'check OK decls=250' "$T/chk.out" && ok "自检 cc_run 绿,decls=250" || bad "自检 cc_run: $(cat "$T/chk.out")"
...
for cv in fnval cloval enumres fnret try tlist own generic gstruct derive optstr boxalias gprobe2 gprobe fmap fs time; do
