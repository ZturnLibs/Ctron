...
1. **调用不校验实参数量**:实参错位会静默误绑(如 4 参调用 5 参函数,形参整体
   右移错位)。写跨函数调用后必须逐参核对;给 call 系加 arity 检查是挂账项。
...
- 发射器能力面 = `selfhosted/fixtures/trans_v0–v3` + 编译器自身;"编译任意
  Ctron 程序"(struct/闭包/并发/插值/值位 if·match 全域)按 selfhosted 排期。
