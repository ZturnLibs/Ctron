...
echo "[3/8] 自举出原生四件套(ctron-cc/chk/emit;冒烟与测试集的原生臂前置)"
sh "$DIR/compiler/native.sh"

echo "[4/8] 验收冒烟(--full,含并发/枚举/fn 值发射与自举固定点)+ tests/ 一致性测试集(对照 C 参考宿主)"
sh "$DIR/compiler/test/smoke.sh" --full
python3 "$DIR/compiler/test/suite.py"

echo "[5/8] ctc 驱动冒烟(run/check/build/help/无 cc 路径)"
sh "$DIR/tests/dist/ctc_smoke.sh"

echo "[6/8] 性能基线冒烟(发射一致性 + 后端加速比)"
sh "$DIR/compiler/bench.sh" 2>&1 | tail -12

echo "[7/8] FFI 边界微基准(跨 C-ABI)"
sh "$DIR/compiler/test/bench_ffi.sh" 2>&1 | tail -16

echo "[8/8] GUI 阶梯(S1–S9 headless:布局桥/事件/绑定竖切/命令缓冲断言/FreeType 中文)"
