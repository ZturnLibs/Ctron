/*
 * ctron_entropy.c - Ctron std/uuid 熵源垫片(P5-B;std/db/c_src 首件)
 *
 * extern 面(std/uuid.ct):int64_t ctron_entropy_fill(int64_t* buf, int64_t n)
 *   → 成功填 n 条【字节 lane】(每 lane 一字节值 0..255;std/net read_t
 *     lane 约定同形)返回 0;失败返回 -1(fail-closed,调用方
 *     uuid_v4/uuid_v7 返回空串,绝不满凑伪熵)。注意 n 计 lane 不计
 *     字节——lane 是 int64 槽,16 lane = 128 字节存储。
 *
 * 放置理由(P5-B 登记):std/uuid 是零 std 依赖叶,而 uuid 消费方 std/db
 * 不得 use std/net(加载器严格互斥树 + 门面 db: StdDb 贯穿口径),垫片
