...
### 3.1.1 I64 值域(v0 实现口径)

类型表名义为定宽整数,v0 自举实现中 `I64` 以**规范十进制文本**承载(可带单个前导 `-`;无前导零;`0` 为唯一零形),发射侧映射 `int64_t`。语义锚点:

- **算术语义**:`+ -` 文本十进制加减;`*` 十进制竖式(符号异或);`/` `%` 为**截断除法**(C99 口径):商向零取整、商符号为操作数符号异或、**余数符号随被除数**(与 §4.5 `%` 语义一致);除零 panic("division by zero",永不关闭,§4.5)。
- **v0 限制(实现口径,非语言承诺)**:
  1. 整数字面量按期望类型宽度门(E2040,§3.7/§10):无后缀字面量(十进制与 `0x/0o/0b` 同口径,下划线分隔符剥离)在 let 注解/赋值/返回/实参四点核对界内,超界拒绝;**I64 期望下超 I32 宽度的字面量直入 I64 域**(十进制取值),comptime 域(const 折叠)同口径。
  2. `I64` 算术实现**溢出检查**(解释侧 2^63 界门 / 发射侧 `__builtin_*_overflow` 帮手,双向一致;`INT64_MIN / -1`、`INT64_MIN % -1`、`-INT64_MIN` 同判溢出,§4.5)。注解位字面量超 int64 宽度由宽度门拒绝(E2040);无注解且超 int64 宽度的字面量参与算术时解释侧/发射侧行为分歧,v0 测试夹具不覆盖。
...
- **局部推断**:`let/var` 绑定、闭包参数(可省)块内推断;泛型调用的类型实参(省略 TypeArgs 时按 §3.9.1 推断,v0.7)。
...
- **泛型 fn**:调用点 TypeArgs **可显式可省略**(v0.7 两段式;显式写法恒合法):
  - **显式**:`render[Pixel](p)`——按声明序对位绑定型参;数量错位报 E2020。
  - **省略(推断)**:对每个型参,**仅从实参类型**统一解出(Go 式;不看返回值上下文、不看链式上下文);字面量实参按默认型别(`0`→I32、`2.5`→F64)。解出冲突 → **E2061**;型参未出现于任何实参位且有具体实参信息 → **E2060**(要求显式)。bound 核对在解出后进行。
  - v1 边界:仅 fn 调用(泛型 struct 字面量保持注解驱动,§3.9.1);不做部分推断(`f[_](…)` 预留)。
  - 每个不同的实参型别组合产生一个特化(`t_名__<实参码>`,如 `t_get__i_s`);同组合多调用点共享一份特化;**推断先于显式化,特化键与单态化机制不受影响**。
